...
import os
from goapi_agent import GoAPIAgent
import dotenv
import logging
...
def initialize_session_state():
    ...
    st.session_state.setdefault("original_api_key",
                                os.environ.get("OPENAI_API_KEY", None))  # Store the original API key
...
def initialize_page():
    st.set_page_config(
        page_title="GO Assistant",
        page_icon="https://avatars.githubusercontent.com/u/5161984?s=200&v=4",
        layout="centered",
        initial_sidebar_state="collapsed",
        menu_items={
            "Get Help": "https://help.geneontology.org/",
            "About": "This application is brought to you by the [Gene Ontology Consortium](https://geneontology.org).",
        }
    )
...
def handle_chat_input():
    if prompt := st.chat_input(disabled=st.session_state.lock_widgets,
                               on_submit=lock_ui):  # Step 4: Add on_submit callback
        ...
        while True:
            try:
                with st.spinner(st.session_state.current_action):
                    ...
                    info = {"session_id": session_id, "message": message.model_dump(),
